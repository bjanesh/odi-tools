...
for filt in filters:
    ...
    for key in tqdm(odi.OTA_dictionary):
        ...
        med_out = image_list.replace('.lis','.mastersflat.raw.fits')
        med_otalist.append(med_out)
        if not os.path.isfile(med_out):
    ...
    for i in tqdm(range(len(med_otalist))):
        ...
        iraf.imfilter.median.setParam('output',med_otalist[i].replace('.raw',''))
        iraf.imfilter.median.setParam('xwindow',31)
        iraf.imfilter.median.setParam('ywindow',31)
